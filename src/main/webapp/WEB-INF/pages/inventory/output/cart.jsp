...
<div id="cart" class="total-section">

	<form:form class="order" method="POST" modelAttribute="inventoryOutput">
...
<script>
	...
	$(document).ready(function() {
		...
		$.ajax({
			async : true,
			type : "GET",
			url : context() + "/inventory/output/items",
			dataType : 'json',
		...
		for (var i = 0; i < item.inputs.length; i++) {
			...
			if(!isDuplicateLot(key)){
				...
				for (var j = 0; j < input.stock; j++) {
					if (input.outputs[0] != undefined
							&& input.outputs[0].qty == (j + 1)) {
				...
				column = '';
				column += "<td class='text-right'>";
				column += "<input name='items["+index+"].inputs["+i+"].notes' type='text' class='form-control' maxlength='40' value='"+input.notes+"' />";
				column += "</td>";
				row += column;					
